...
The verification page also displays the latest eight journal entries for its selected scenario. This is a bounded read-only view of already-recorded facts; recording still requires the explicit confirmation action and no dashboard control can alter or remove history.
