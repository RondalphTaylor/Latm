...
The verification page displays eight journal entries at a time for its selected scenario and uses a ninth bounded row only to determine whether a next page exists. Its prior/next links preserve the optional fingerprint being compared, and each entry links to a scenario-scoped read-only detail page that exposes its recorded result and policy provenance. This is a bounded read-only view of already-recorded facts; recording still requires the explicit confirmation action and no dashboard control can alter or remove history.
