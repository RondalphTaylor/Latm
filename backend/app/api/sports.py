...
def get_sports_data_provider(
    ...
    """Build the configured authenticated read-only sports adapter."""
    if settings.sports_data_provider is SportsDataProviderName.BALLDONTLIE:
        if (
...
def get_sports_ingestion_service(
    ...
    """Build the NBA sports-ingestion coordinator."""
...
async def ingest_teams(
    ...
    """Fetch authenticated NBA team data and persist normalized records."""
    try:
        result = await service.ingest_teams()
    except SportsDataProviderError as exc:
        logger.warning("NBA team ingestion failed safely: %s", exc)
...
async def list_teams(
    ...
    """List persisted normalized NBA teams."""
...
async def ingest_events(
    ...
    """Fetch and persist NBA games within an inclusive bounded date range."""
    try:
        ...
        logger.warning("NBA event ingestion failed safely: %s", exc)
        raise _provider_failure(exc) from exc
...
async def list_events(
    ...
    """List persisted NBA games using provider-neutral filters."""
...
async def get_event(
    ...
    """Return one persisted NBA game by stable internal ID."""
