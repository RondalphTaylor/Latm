...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("mark_price", sa.Numeric(8, 6)),
        ...
        sa.Column("official_resolution_id", postgresql.UUID(as_uuid=True)),
        ...
        sa.ForeignKeyConstraint(["entry_id"], ["nfl_pilot_entries.id"], ondelete="RESTRICT"),
        ...
        sa.ForeignKeyConstraint(
            ["official_resolution_id"], ["market_resolutions.id"], ondelete="RESTRICT"
        ),
        ...
        sa.CheckConstraint(
            "total_cost_basis > 0 AND market_value >= 0", name="ck_nfl_pilot_positions_values"
        ),
        sa.CheckConstraint("jsonb_typeof(audit) = 'object'", name="ck_nfl_pilot_positions_audit"),
    )
    op.create_table(
        ...
        sa.Column("market_price_id", postgresql.UUID(as_uuid=True)),
        sa.Column("official_resolution_id", postgresql.UUID(as_uuid=True)),
        ...
        sa.UniqueConstraint("idempotency_key", name="uq_nfl_pilot_position_events_idempotency"),
        sa.UniqueConstraint(
    ...
    op.create_index(
        "ix_nfl_pilot_position_events_position_recorded",
        "nfl_pilot_position_events",
